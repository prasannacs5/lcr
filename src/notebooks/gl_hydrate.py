...
# Catalog already exists: banking_cfo_treasury
spark.sql("CREATE SCHEMA IF NOT EXISTS banking_cfo_treasury.aura_bank")
...
spark.sql("""
CREATE TABLE IF NOT EXISTS banking_cfo_treasury.aura_bank.haircuts (
    id BIGINT,
...
haircuts_count = spark.table("banking_cfo_treasury.aura_bank.haircuts").count()
...
if haircuts_count == 0:
    ...
    haircuts_df.write.mode("append").saveAsTable("banking_cfo_treasury.aura_bank.haircuts")
...
spark.sql("""
CREATE TABLE IF NOT EXISTS banking_cfo_treasury.aura_bank.runoff_rates (
    id BIGINT,
...
runoff_count = spark.table("banking_cfo_treasury.aura_bank.runoff_rates").count()
...
if runoff_count == 0:
    ...
    runoff_df.write.mode("append").saveAsTable("banking_cfo_treasury.aura_bank.runoff_rates")
...
try:
    existing_gl = spark.table("banking_cfo_treasury.aura_bank.general_ledger")
    table_exists = True
...
if not table_exists or existing_count == 0:
    ...
    spark.sql("""
    CREATE TABLE IF NOT EXISTS banking_cfo_treasury.aura_bank.general_ledger (
        gl_account STRING,
        account_name STRING,
        category STRING,
        account_type STRING,
    ...
    gl_df.write.mode("append").saveAsTable("banking_cfo_treasury.aura_bank.general_ledger")
    ...
    new_gl_df.write.mode("append").saveAsTable("banking_cfo_treasury.aura_bank.general_ledger")
...
# ============================================
gl_final = spark.table("banking_cfo_treasury.aura_bank.general_ledger")
...
print("="*80)
display(spark.table("banking_cfo_treasury.aura_bank.haircuts"))
...
print("="*80)
display(spark.table("banking_cfo_treasury.aura_bank.runoff_rates"))
...
print("="*80)
gl_df = spark.table("banking_cfo_treasury.aura_bank.general_ledger").orderBy("category", "gl_account")
display(gl_df)
...
# MAGIC ```sql
# MAGIC DROP TABLE banking_cfo_treasury.aura_bank.general_ledger;
# MAGIC ```
