...
# MAGIC
# MAGIC ## Function: `banking_cfo_treasury.aura_bank.calculate_lcr()`
# MAGIC
# MAGIC ### Purpose
...
# MAGIC ### Signature
# MAGIC ```sql
# MAGIC banking_cfo_treasury.aura_bank.calculate_lcr(as_of_date TIMESTAMP)
# MAGIC ```
# MAGIC
# MAGIC ### Returns
...
# MAGIC    * Joins [general_ledger](#table/banking_cfo_treasury.aura_bank.general_ledger) with [haircuts](#table/banking_cfo_treasury.aura_bank.haircuts) table
...
# MAGIC    * Joins [general_ledger](#table/banking_cfo_treasury.aura_bank.general_ledger) with [runoff_rates](#table/banking_cfo_treasury.aura_bank.runoff_rates) table
...
# MAGIC
# MAGIC ## Overview
# MAGIC This notebook provides a complete system for calculating and tracking the **Liquidity Coverage Ratio (LCR)** for Aura Bank using data from the [banking_cfo_treasury.aura_bank](#table/banking_cfo_treasury.aura_bank) schema.
# MAGIC
...
# MAGIC * **[banking_cfo_treasury.aura_bank.lcr_results](#table/banking_cfo_treasury.aura_bank.lcr_results)** - Main LCR snapshots
# MAGIC * **[banking_cfo_treasury.aura_bank.hqla_breakdown](#table/banking_cfo_treasury.aura_bank.hqla_breakdown)** - Detailed asset breakdown
# MAGIC * **[banking_cfo_treasury.aura_bank.cash_outflows_breakdown](#table/banking_cfo_treasury.aura_bank.cash_outflows_breakdown)** - Detailed liability breakdown
# MAGIC
...
# MAGIC **`banking_cfo_treasury.aura_bank.capture_lcr_snapshot(as_of_date TIMESTAMP)`**
...
# MAGIC SELECT banking_cfo_treasury.aura_bank.calculate_lcr(
# MAGIC   (SELECT MAX(timestamp) FROM banking_cfo_treasury.aura_bank.general_ledger)
# MAGIC ) as lcr_result;
# MAGIC
# MAGIC -- Capture a snapshot (only creates if one doesn't exist for the day)
# MAGIC CALL banking_cfo_treasury.aura_bank.capture_lcr_snapshot(
# MAGIC   (SELECT MAX(timestamp) FROM banking_cfo_treasury.aura_bank.general_ledger)
# MAGIC );
# MAGIC
# MAGIC -- View all snapshots
# MAGIC SELECT * FROM banking_cfo_treasury.aura_bank.lcr_results ORDER BY calculation_timestamp DESC;
# MAGIC ```
# MAGIC
# MAGIC ## Data Sources
# MAGIC * **[banking_cfo_treasury.aura_bank.general_ledger](#table/banking_cfo_treasury.aura_bank.general_ledger)** - Account balances and classifications
# MAGIC * **[banking_cfo_treasury.aura_bank.haircuts](#table/banking_cfo_treasury.aura_bank.haircuts)** - Regulatory haircut rates for assets
# MAGIC * **[banking_cfo_treasury.aura_bank.runoff_rates](#table/banking_cfo_treasury.aura_bank.runoff_rates)** - Runoff rates for liabilities

# COMMAND ----------

# MAGIC %md
# MAGIC ## Exploring banking_cfo_treasury.aura_bank Schema Tables
# MAGIC
...
def capture_lcr_snapshot_once_per_day(as_of_date):
    ...
    existing_snapshot = spark.sql(f"""
        ...
        FROM banking_cfo_treasury.aura_bank.lcr_results
    ...
    result = spark.sql(f"""
        CALL banking_cfo_treasury.aura_bank.capture_lcr_snapshot(TIMESTAMP '{date_str}')
    """).collect()[0]
...
existing_count = results_df.filter('is_new = false AND status NOT LIKE "%ERROR%"').count()
error_count = results_df.filter('status LIKE "%ERROR%"').count()
print(f"Existing snapshots found: {existing_count}")
...
# MAGIC FROM banking_cfo_treasury.aura_bank.general_ledger
...
# MAGIC FROM banking_cfo_treasury.aura_bank.haircuts
...
# MAGIC   regulatory_logic
# MAGIC FROM banking_cfo_treasury.aura_bank.runoff_rates
...
# MAGIC CREATE OR REPLACE FUNCTION banking_cfo_treasury.aura_bank.calculate_lcr(
...
# MAGIC     LEFT JOIN banking_cfo_treasury.aura_bank.haircuts h 
...
# MAGIC     FROM filtered_gl gl
# MAGIC     LEFT JOIN banking_cfo_treasury.aura_bank.runoff_rates r 
...
# MAGIC         ELSE CAST(NULL AS DOUBLE)
# MAGIC       END AS DOUBLE) as lcr_ratio,
# MAGIC       CAST(ROUND(COALESCE(ah.total_hqla, CAST(0 AS DOUBLE)), 2) AS DOUBLE) as hqla_total,
# MAGIC       CAST(ROUND(COALESCE(lo.total_outflows, CAST(0 AS DOUBLE)), 2) AS DOUBLE) as net_cash_outflows,
# MAGIC       CAST(as_of_date AS TIMESTAMP) as calculation_date
# MAGIC     )
...
# MAGIC )
# MAGIC SELECT 
# MAGIC   banking_cfo_treasury.aura_bank.calculate_lcr(max_date).lcr_ratio,
# MAGIC   banking_cfo_treasury.aura_bank.calculate_lcr(max_date).hqla_total,
# MAGIC   banking_cfo_treasury.aura_bank.calculate_lcr(max_date).net_cash_outflows,
# MAGIC   banking_cfo_treasury.aura_bank.calculate_lcr(max_date).calculation_date,
# MAGIC   CASE 
# MAGIC     WHEN banking_cfo_treasury.aura_bank.calculate_lcr(max_date).lcr_ratio >= 1.0 THEN 'COMPLIANT (≥100%)'
...
# MAGIC FROM banking_cfo_treasury.aura_bank.general_ledger gl
# MAGIC LEFT JOIN banking_cfo_treasury.aura_bank.haircuts h 
# MAGIC   ON gl.account_type = h.asset_category
...
# MAGIC SELECT 
# MAGIC   banking_cfo_treasury.aura_bank.calculate_lcr(TIMESTAMP '2026-02-19 03:10:08.816') as lcr_result
...
# MAGIC SELECT 
# MAGIC   timestamp,
# MAGIC   banking_cfo_treasury.aura_bank.calculate_lcr(timestamp).lcr_ratio as lcr_ratio,
# MAGIC   banking_cfo_treasury.aura_bank.calculate_lcr(timestamp).hqla_total as hqla_total,
# MAGIC   banking_cfo_treasury.aura_bank.calculate_lcr(timestamp).net_cash_outflows as net_cash_outflows,
# MAGIC   CASE 
# MAGIC     WHEN banking_cfo_treasury.aura_bank.calculate_lcr(timestamp).lcr_ratio >= 1.0 THEN 'COMPLIANT'
...
# MAGIC CREATE TABLE IF NOT EXISTS banking_cfo_treasury.aura_bank.lcr_results (
...
# MAGIC CREATE TABLE IF NOT EXISTS banking_cfo_treasury.aura_bank.hqla_breakdown (
...
# MAGIC CREATE TABLE IF NOT EXISTS banking_cfo_treasury.aura_bank.cash_outflows_breakdown (
# MAGIC   snapshot_id STRING COMMENT 'Links to lcr_results.snapshot_id',
# MAGIC   calculation_timestamp TIMESTAMP COMMENT 'When this calculation was performed',
...
# MAGIC CREATE OR REPLACE PROCEDURE banking_cfo_treasury.aura_bank.capture_lcr_snapshot(
# MAGIC   as_of_date TIMESTAMP
# MAGIC )
# MAGIC LANGUAGE SQL
...
# MAGIC   INSERT INTO banking_cfo_treasury.aura_bank.lcr_results
# MAGIC   SELECT 
# MAGIC     snapshot_id,
# MAGIC     calc_timestamp as calculation_timestamp,
# MAGIC     as_of_date,
# MAGIC     banking_cfo_treasury.aura_bank.calculate_lcr(as_of_date).lcr_ratio as lcr_ratio,
# MAGIC     banking_cfo_treasury.aura_bank.calculate_lcr(as_of_date).hqla_total as hqla_total,
# MAGIC     banking_cfo_treasury.aura_bank.calculate_lcr(as_of_date).net_cash_outflows as net_cash_outflows,
# MAGIC     CASE 
# MAGIC       WHEN banking_cfo_treasury.aura_bank.calculate_lcr(as_of_date).lcr_ratio >= 1.0 THEN 'COMPLIANT'
# MAGIC       WHEN banking_cfo_treasury.aura_bank.calculate_lcr(as_of_date).lcr_ratio IS NULL THEN 'NO_DATA'
# MAGIC       ELSE 'NON-COMPLIANT'
# MAGIC     END as regulatory_status,
# MAGIC     CURRENT_USER() as created_by;
# MAGIC   
# MAGIC   -- Insert HQLA breakdown
# MAGIC   INSERT INTO banking_cfo_treasury.aura_bank.hqla_breakdown
# MAGIC   SELECT 
# MAGIC     snapshot_id,
# MAGIC     calc_timestamp as calculation_timestamp,
# MAGIC     as_of_date,
...
# MAGIC   INSERT INTO banking_cfo_treasury.aura_bank.cash_outflows_breakdown
# MAGIC   SELECT 
# MAGIC     snapshot_id,
# MAGIC     calc_timestamp as calculation_timestamp,
# MAGIC     as_of_date,
...
# MAGIC   created_by
# MAGIC FROM banking_cfo_treasury.aura_bank.lcr_results
...
# MAGIC FROM banking_cfo_treasury.aura_bank.hqla_breakdown h
...
# MAGIC FROM banking_cfo_treasury.aura_bank.cash_outflows_breakdown c
...
# MAGIC FROM banking_cfo_treasury.aura_bank.lcr_results r
...
# MAGIC
# MAGIC 1. **[banking_cfo_treasury.aura_bank.lcr_results](#table/banking_cfo_treasury.aura_bank.lcr_results)**
...
# MAGIC
# MAGIC 2. **[banking_cfo_treasury.aura_bank.hqla_breakdown](#table/banking_cfo_treasury.aura_bank.hqla_breakdown)**
...
# MAGIC
# MAGIC 3. **[banking_cfo_treasury.aura_bank.cash_outflows_breakdown](#table/banking_cfo_treasury.aura_bank.cash_outflows_breakdown)**
...
# MAGIC CALL banking_cfo_treasury.aura_bank.capture_lcr_snapshot(TIMESTAMP '2026-02-19 03:10:08.816');
# MAGIC
# MAGIC -- View latest snapshot
# MAGIC SELECT * FROM banking_cfo_treasury.aura_bank.lcr_results ORDER BY calculation_timestamp DESC LIMIT 1;
# MAGIC
# MAGIC -- View HQLA breakdown for a specific snapshot
# MAGIC SELECT * FROM banking_cfo_treasury.aura_bank.hqla_breakdown WHERE snapshot_id = 'LCR_20260219_032856_1771471736959';
# MAGIC
# MAGIC -- View cash outflows breakdown for a specific snapshot
# MAGIC SELECT * FROM banking_cfo_treasury.aura_bank.cash_outflows_breakdown WHERE snapshot_id = 'LCR_20260219_032856_1771471736959';
# MAGIC ```
...
# MAGIC
# MAGIC 1. **Checks for existing snapshot**: Queries `banking_cfo_treasury.aura_bank.lcr_results` to see if a snapshot already exists for the date (comparing only the date part, not timestamp)
...
# MAGIC latest_date = spark.sql("SELECT MAX(timestamp) FROM banking_cfo_treasury.aura_bank.general_ledger").collect()[0][0]
...
# MAGIC | `banking_cfo_treasury.aura_bank.capture_lcr_snapshot()` | SQL procedure (called by Python wrapper) |
# MAGIC
# MAGIC ### Verification
# MAGIC
