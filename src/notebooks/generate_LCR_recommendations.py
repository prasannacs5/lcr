...
spark.sql("""
  CREATE TABLE IF NOT EXISTS banking_cfo_treasury.aura_bank.reco_table (
    timestamp TIMESTAMP,
    tag STRING,
    text STRING
  )
...
print("Table banking_cfo_treasury.aura_bank.reco_table created/verified successfully")
...
def get_aura_bank_data():
    """
    Query all tables under banking_cfo_treasury.aura_bank schema and format as context
    """
    # Get list of tables in the schema
    tables = spark.sql("SHOW TABLES IN banking_cfo_treasury.aura_bank").collect()
    
    context_data = []
    
    for table_row in tables:
        table_name = table_row['tableName']
        full_table_name = f"banking_cfo_treasury.aura_bank.{table_name}"
...
def call_gemini_model(prompt, analysis_type):
    ...
    # Get all data from banking_cfo_treasury.aura_bank schema
    print(f"Fetching data from banking_cfo_treasury.aura_bank schema...")
    context_data = get_aura_bank_data()
    
    # Format context for the model
    context_text = "# Liquidity Coverage Ratio (LCR) Data Context\n\n"
    context_text += "You are analyzing banking liquidity data from the banking_cfo_treasury.aura_bank schema. Below is the available data:\n\n"
...
spark.sql(f"""
  INSERT INTO banking_cfo_treasury.aura_bank.reco_table (timestamp, tag, text)
...
latest_recs = spark.sql("""
  ...
  FROM banking_cfo_treasury.aura_bank.reco_table
...
# MAGIC * **Data Context**: All tables from `banking_cfo_treasury.aura_bank` schema automatically loaded
...
# MAGIC * Automatically queries all tables in `banking_cfo_treasury.aura_bank` schema
...
# MAGIC
# MAGIC ### Output
# MAGIC * All analyses stored in `banking_cfo_treasury.aura_bank.reco_table`
