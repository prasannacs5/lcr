...
def stream_agent(user_message: str):
    ...
    for turn in range(5):
        ...
        for tc in tool_calls:
            ...
            # Emit chain-of-thought events: SQL query + data retrieved
            _emit_tool_events(tool_result, fn_name, question, tc.get("id", ""))
            sql_text, data_text = _parse_tool_result(tool_result)
            yield {
                "type": "tool_call",
                "tool_name": fn_name,
                "sql": sql_text,
...
def _parse_tool_result(tool_result: str) -> tuple:
    """Parse tool result to extract SQL and data portions."""
    import re
    sql_text = ""
    data_text = ""
    ...
    # Format from _query_sql_direct: "SQL: <query>\n\nResults (N rows):\n<table>"
    if tool_result.startswith("SQL:"):
        parts = tool_result.split("\n\nResults", 1)
        sql_text = parts[0].replace("SQL: ", "", 1).strip()
        if len(parts) > 1:
            data_text = ("Results" + parts[1]).strip()
        return (sql_text, data_text)

    # Format from _query_genie / _extract_genie_response: may have ```sql blocks
    sql_match = re.search(r"```sql\n(.*?)```", tool_result, re.DOTALL)
    if sql_match:
        sql_text = sql_match.group(1).strip()
        data_text = re.sub(r"```sql\n.*?```\n*", "", tool_result, flags=re.DOTALL).strip()
        return (sql_text, data_text)

    # No SQL found — return the whole thing as data
    return ("", tool_result[:1000])


def _emit_tool_events(tool_result, fn_name, question, call_id):
    """Log tool events for debugging."""
    logger.info("Tool %s called with question: %s", fn_name, question[:200])
